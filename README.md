...
- `documents/resident_card.py`: 주민등록증 번호·발급일 후보 추출
...
발급일은 점으로 구분한 연·월·일 단독 또는 발급일 라벨이 붙은 단일 OCR 항목에서 추출한다.
후보가 하나이고 점수가 0.90~1.0이며 실제 존재하는 오늘 이전 또는 오늘 날짜이면 YYYY-MM-DD로 표시한다.
미래 날짜는 PC의 오늘 날짜를 기준으로 판단한다. 날짜 형식으로 찾은 후보이며 실제 발급일인지 확정하지 않는다.
번호와 발급일은 독립적으로 추출하며, 직접 수정할 수 있고 재인식이나 새 이미지 선택 시 초기화된다.
...
- 주민등록증 후보의 번호·발급일 추출과 편집창 자동 입력

다른 카드 종류 판별, 이름·주소 자동 입력, 항목 검증, 결과 저장은 아직 구현하지 않았다.
