...
- `documents/resident_card.py`: 주민등록증 번호 형식 추출
...
주민등록증 후보에서는 번호 형식이 하나이고 OCR 점수가 0.90~1.0이면 번호 입력창에 자동 표시한다.
번호와 선택적인 주민등록번호 라벨만 있는 단일 OCR 항목을 처리하며, 분리된 항목은 합치지 않는다.
후보 누락·중복·낮은 점수는 자동 입력을 보류한다. 형식 추출이며 생년월일·번호 유효성·진위 검증은 아니다.
번호는 직접 수정할 수 있으며 재인식이나 새 이미지 선택 시 초기화된다.
현재 원본 글자에는 배경 문구도 포함될 수 있다. 이름·주소 자동 입력은 아직 연결하지 않았다.
...
- 주민등록증 후보의 번호 형식 추출과 편집창 자동 입력

다른 카드 종류 판별, 이름·주소·발급일 자동 입력, 항목 검증, 결과 저장은 아직 구현하지 않았다.
